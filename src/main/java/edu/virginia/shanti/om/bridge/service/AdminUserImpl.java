...
//	
	...
	@Value("${sakaiAdminSecret}")
	private String sakaiAdminSecret;
...
//		Authentication auth = new UsernamePasswordAuthenticationToken(sakaiAdminUser,sessionString, Arrays.asList(new GrantedAuthority[] { new GrantedAuthorityImpl("sakaiAdmin") }));
		...
		// new authentication method uses a secret rather than an admin session
		Authentication auth = new UsernamePasswordAuthenticationToken("", sakaiAdminSecret, Arrays.asList(new GrantedAuthority[] { new GrantedAuthorityImpl("sakaiAdmin") }));
...
//		}
		...
		// This function becomes degenerate with the new admin secret method of authentication.
